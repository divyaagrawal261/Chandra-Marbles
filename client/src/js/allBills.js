...
const billsUrl=`${apiURL}/api/sale/`;
...
fetch(billsUrl+"all",{
    ...
    data.forEach(bill=>{
        ...
        <div class="row">
            <div class="col-4">Employee: ${bill.employeeName}</div>
            <div class="col-4">Date: ${new Date(bill.date).toLocaleDateString('en-GB', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
              })}</div>
            <div class="col-4">Customer: ${bill.phone}</div>
        </div>
        </div>
        <hr>`
        ...
        products.forEach(product=>{
            const amount=Number(product.rate)*Number(product.orderedQuantity);
            const billedProduct=`<div class="row productRow">
            <div class="col-4">${product.barcode}</div>
            <div class="col-3">${product.orderedQuantity}</div>
        ...
        block2.innerHTML=`<div class="container amountContainer"><br>
        <div class="row">Total Amount: ${bill.totalAmount}</div>
        <div class="row">Discount: ${bill.discount}</div>
        <div class="row">Paid: ${bill.paid}</div>
        <div class="row">Balance: ${bill.balance}</div><br>
...
{
    fetch(`http://localhost:5001/api/sale/delete/${id}`,{
        method:"DELETE",
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
